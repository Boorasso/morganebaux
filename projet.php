...
//Récupérer et tester la variable récupérée en GET !
$ref_projet = 1;
...
$query_projet = $bdd->prepare('SELECT `nom_projet`, `poste`, `texte` FROM `projets` WHERE `id_projet`= ?');
...
//Image principale :
$query_main_image = $bdd->prepare('SELECT `url_img`, `alt_img` FROM `images` WHERE `idprojet`= ? AND `pos_img` = 1');
$query_main_image->execute(array($ref_projet));
$main_image_projet = $query_main_image->fetch();
$query_main_image->closeCursor();

//Images projet :
$query_images = $bdd->prepare('SELECT `url_img` FROM `images` WHERE `idprojet`= ? AND `pos_img` > 1 ORDER BY `pos_img`');
